...
interface ConsoleRecoveryQueueProps {
  onSelectTransaction: (tx: TransactionEvaluation) => void;
  atRiskCount?: number;
}

export const ConsoleRecoveryQueue: React.FC<ConsoleRecoveryQueueProps> = ({ 
  onSelectTransaction,
  atRiskCount = 2026,
}) => {
  ...
  return (
    <section id="recovery-queue-console" className="py-16 px-4 sm:px-8 lg:px-14 border-b border-white/[0.06] revora-bg-grid">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-950/40 border border-indigo-800/40 text-xs font-mono text-indigo-300 mb-4">
            ...
            <span>06 / OPERATIONS CONSOLE</span>
          </div>
          <div className="flex items-baseline gap-4">
            <h2 className="section-display-title text-white tracking-tight uppercase">
              RECOVERY QUEUE
            </h2>
            <span className="text-sm font-mono font-bold text-rose-400 bg-rose-950/60 border border-rose-800/60 px-3 py-1 rounded-full">
        ...
        {hasActiveFilters && (
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-mono transition-colors cursor-pointer self-start md:self-auto"
          >
      ...
      {/* Console Filter Bar */}
      <div className="glass-panel rounded-2xl p-4 border border-white/[0.08] mb-6 space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-950/80 border border-white/[0.08] rounded-xl text-xs font-mono text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
            />
          </div>
          ...
          <select
            ...
            className="px-3 py-2 bg-slate-950/80 border border-white/[0.08] rounded-xl text-xs font-mono text-slate-300 focus:outline-none focus:border-indigo-500 transition-colors cursor-pointer"
          >
        ...
        {hasActiveFilters && (
          <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-white/[0.04]">
            ...
            {debouncedSearch && (
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-mono bg-slate-800 text-slate-200 border border-slate-700">
      ...
      {/* High-End Operations Console Table */}
      <div className="glass-panel rounded-3xl border border-white/[0.08] overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/[0.06] bg-slate-950/60 text-[10px] font-mono uppercase tracking-widest text-slate-400">
                ...
                <th className="py-4 px-6 text-right">Audit Trace</th>
              </tr>
            </thead>
        ...
        {/* Pagination Footer */}
        <div className="p-4 bg-slate-950/80 border-t border-white/[0.06] flex items-center justify-between text-xs font-mono text-slate-400">
          <div>
